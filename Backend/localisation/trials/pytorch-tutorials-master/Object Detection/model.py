...
class ProposalModule(nn.Module):
    ...
    def forward(self, feature_map, pos_anc_ind=None, neg_anc_ind=None,
                pos_anc_coords=None):
        """ DOCSTRING """
        # determine mode
        if pos_anc_ind is None or neg_anc_ind is None or \
                pos_anc_coords is None:
            mode = 'eval'
        else:
            mode = 'train'
...
class RegionProposalNetwork(nn.Module):
    ...
    def forward(self, images, gt_bboxes, gt_classes):
        ...
        anc_base = gen_anc_base(
            anc_pts_x, anc_pts_y, self.anc_scales, self.anc_ratios, (
                self.out_h, self.out_w))
        anc_boxes_all = anc_base.repeat(batch_size, 1, 1, 1, 1)

        # get positive and negative anchors amongst other things
        gt_bboxes_proj = project_bboxes(
            gt_bboxes, self.width_scale_factor,
        ...
        conf_scores_pos, conf_scores_neg, offsets_pos, proposals = \
            self.proposal_module(feature_map, positive_anc_ind,
    ...
    def inference(self, images, conf_thresh=0.5, nms_thresh=0.7):
        ...
        with torch.no_grad():
            ...
            # filter out proposals based on conf threshold and nms threshold
            # for each image
            proposals_final = []
            conf_scores_final = []
...
class ClassificationModule(nn.Module):
    ...
    def __init__(self, out_channels, n_classes, roi_size, hidden_dim=512,
        ...
        self.f_c = nn.Linear(out_channels, hidden_dim)
        self.dropout = nn.Dropout(p_dropout)
...
class TwoStageDetector(nn.Module):
    ...
    def inference(self, images, conf_thresh=0.5, nms_thresh=0.7):
        ...
        for i in range(batch_size):
            ...
            classes_final.append(classes_all[c_counter: c_counter+n_proposals])
            c_counter += n_proposals
